...
import com.netease.nim.uikit.http.BaseCallback;
import com.netease.nim.uikit.http.HttpBaseBean;
import com.netease.nim.uikit.http.RetrofitFactory;
import com.netease.nim.uikit.http.WichStep;
...
public class MessageFragment extends TFragment implements ModuleProxy, View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        String msgTitle = arguments.getString("msgTitle");
        tvFinishMsg = (TextView) rootView.findViewById(R.id.tv_finishMsg);
        getViSiBle();
        tvFinishMsg.setOnClickListener(this);
        return rootView;
    }

    private void getViSiBle() {
        String baseUrl = "/api/interrogation/getStep/"+recordId;
        final Call<HttpBaseBean<WichStep>> whichStep = RetrofitFactory.getInstance(getActivity()).getWhichStep(baseUrl, recordId);
        new BaseCallback(whichStep).handleResponse(new BaseCallback.ResponseListener<WichStep>() {
            ...
            public void onSuccess(WichStep wichStep, String info) {
                if (wichStep!=null) {
                    ...
                    if (step == 0) {
                        //填写完成
                        tvFinishMsg.setVisibility(View.GONE);
                    } else if (step == 1) {
                        tvFinishMsg.setVisibility(View.VISIBLE);
                    } else if (step == 2) {
                        tvFinishMsg.setVisibility(View.VISIBLE);
                    }
                }
            ...
            public void onSuccess(WichStep wichStep, String info) {
                if (wichStep!=null) {
                    ...
                    if (step == 0) {
                        ...
                        ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.PatientInfomationActivity");
                        ...
                        if (template.equals("儿童")){
                            ...
                            if (two_step == 1) {
                                Intent intent = new Intent(Intent.ACTION_MAIN);
                                intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteChildFromFirstActivity");
                                intent.setComponent(componentName);
                                intent.putExtra("recordId", recordId);
                                startActivity(intent);
                            } else if (two_step == 2) {
                                Intent intent = new Intent(Intent.ACTION_MAIN);
                                intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteChildFromSecondActivity");
                                intent.setComponent(componentName);
                                intent.putExtra("recordId", recordId);
                                startActivity(intent);
                            } else if (two_step == 3) {
                                Intent intent = new Intent(Intent.ACTION_MAIN);
                                intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteChildFromThirdActivity");
                                intent.setComponent(componentName);
                                intent.putExtra("recordId", recordId);
                                startActivity(intent);
                            }
                            ...
                            if (step2==0){
                                ...
                                if (two_step==1){
                                    Intent intent = new Intent(Intent.ACTION_MAIN);
                                    intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                    ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteFromFirstActivity");
                                    intent.setComponent(componentName);
                                    intent.putExtra("recordId",recordId);
                                    startActivity(intent);
                                }else if (two_step==2){
                                    ...
                                    intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                    intent.putExtra("recordId",recordId);
                                    ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteFromSecondActivity");
                                    intent.setComponent(componentName);
                                    startActivity(intent);
                                }else if (two_step==3){
                                    Intent intent = new Intent(Intent.ACTION_MAIN);
                                    intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                    ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteFromThirdActivity");
                                    intent.setComponent(componentName);
                                    intent.putExtra("recordId",recordId);
                                    startActivity(intent);
                                }else if (two_step==4){
                                    Intent intent = new Intent(Intent.ACTION_MAIN);
                                    intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                    ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteFromFourActivity");
                                    intent.setComponent(componentName);
                                    intent.putExtra("recordId",recordId);
                                    startActivity(intent);
                                }else if (two_step==5){
                                    ...
                                    if (sex.equals("M")){
                                        Intent intent = new Intent(Intent.ACTION_MAIN);
                                        intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                        ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteFifthManActivity");
                                        intent.setComponent(componentName);
                                        intent.putExtra("recordId",recordId);
                                        startActivity(intent);
                                        ...
                                        Intent intent = new Intent(Intent.ACTION_MAIN);
                                        intent.addCategory(Intent.CATEGORY_LAUNCHER);
                                        ComponentName componentName = new ComponentName("com.ais.patient","com.ais.patient.activity.main.WriteFifthActivity");
                                        intent.setComponent(componentName);
                                        intent.putExtra("recordId",recordId);
                                        startActivity(intent);
                                    }
    ...
    public void onClick(View v) {
        ...
        intent.putExtra("doctorId",doctorId);
        intent.putExtra("recordId",recordId);
        ComponentName cn = new ComponentName(getActivity(), "com.ais.patient.activity.main.PatientInfomationActivity");
    ...
    protected boolean isAllowSendMessage(final IMMessage message) {
        return true;//return customization.isAllowSendMessage(message);
    }
