                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (i == IAgoraAPI.ECODE_LOGOUT_E_KICKED) { // other login the account
                            Toast.makeText(CallActivity.this, "账号已经在别的设备登录", Toast.LENGTH_SHORT).show();

                        } else if (i == IAgoraAPI.ECODE_LOGOUT_E_NET) { // net
                            Toast.makeText(CallActivity.this, "网络不通畅", Toast.LENGTH_SHORT).show();
                            finish();
                        }
...
//                      "status": 1 // Busy
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (s2.contains("status") && s2.contains("1")) {
                            Toast.makeText(CallActivity.this, account + " 对方正忙", Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(CallActivity.this, account + " 对方已拒绝", Toast.LENGTH_SHORT).show();
                        }
    ...
    protected void onDestroy() {
        ...
        mRtcEngine = null;
        Intent intent = new Intent();
        intent.setAction(Constant.ACTION_VIDEO_CALL_BACK);
    ...
    private void initializeAgoraEngine() {
        ...
        mRtcEngine = MyApplication.getInstance().getmRtcEngine();
